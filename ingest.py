# pip install -q pymupdf tiktoken supabase openai tqdm python-dotenv

# ingest.py
#pip install pymupdf tiktoken supabase sentence-transformers tqdm python-dotenv sentence-transformers
...
EMBED_MODEL = "nomic-ai/nomic-embed-text-v1.5"  # 1536 dims → matches your table
BATCH_EMBED = 100
BATCH_INSERT = 200
...
def chunk_page_by_sentences(
    ...
    while i < len(sents):
        ...
        while max_tokens and len(ids) > max_tokens and len(piece) > 1:
            piece = piece[:-1]  # remove the last sentence
            chunk = " ".join(piece)
            ids = enc.encode(chunk)
...
def main():
    ...
    ).tolist()  # Convert numpy array to list of lists
